...
let results = exports.results = {};

// @deprecated due to TypeForm v2.0
const calcScore = exports.calcScore = latestAnsweredItems => {
  ...
  for (let i = 0; i < latestAnsweredItems.length; i++) {
    ...
    for (let i = 0; i < econ.length; i++) {
      if (econ[i].indexOf('beneficial') !== -1) econScore++;else if (econ[i].startsWith(' ') && econ[i].indexOf('Agree')) econScore++;else if (!econ[i].startsWith(' ') && econ[i].indexOf('Disagree')) econScore++;
    }
    ...
    for (let i = 0; i < soc.length; i++) {
      if (soc[i].indexOf('embedded') !== -1) socScore++;else if (soc[i].startsWith(' ') && soc[i].indexOf('Agree')) socScore++;else if (!soc[i].startsWith(' ') && soc[i].indexOf('Disagree')) socScore++;
    }
